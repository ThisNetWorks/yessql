...
{
    public abstract class UnaryEngineBuilder<T, TTermOption> : OperatorEngineBuilder<T, TTermOption> where T : class where TTermOption : TermOption
    {
    ...
    public class DocumentUnaryEngineBuilder<T> : UnaryEngineBuilder<T, EnumerableTermOption<T>> where T : class
    {
        public DocumentUnaryEngineBuilder(string name, Func<string, T, EnumerableExecutionContext<T>, ValueTask<T>> query) : base(new EnumerableTermOption<T>(name, query))
        {
        }
    }
}
